...
- **Тексты**: секция `[bot_messages]` и env `TELEMT_ADMIN__BOT_MESSAGES__*` (см. `src/config.rs`). Помимо `/start`, можно переопределять шаблон ссылки `{link}`, текст авто-одобрения, сообщения вокруг рассылки, а также тексты главного экрана (`admin_home_text`), user home для разных статусов (`user_home_approved`, `user_home_pending`, `user_home_rejected`, `user_home_deleted`), инструкцию по подключению (`usage_guide_text`), справку (`help_admin_text`, `help_user_text`), сообщение о неизвестной команде (`unknown_command_text`), ошибки токенов (`token_error_not_found`, `token_error_revoked`, `token_error_expired`, `token_error_usage_limit`), подтверждение действий над сервисом (`service_action_confirm_template`) и шаблоны уведомлений администраторам (`admin_notify_new_request_template`, `admin_notify_auto_approve_template`). Доступные плейсхолдеры: `{link}`, `{action}`, `{id}`, `{user_id}`, `{username}`, `{display_name}`, `{time}`, `{invite_token_id}`, `{ok}`, `{failed}`, `{total}`.
